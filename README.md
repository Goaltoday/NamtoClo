...
## Tone3000 real-time NAM preview (v2.9.1)

When a Tone3000 NAM is loaded, the application looks for `Power - Guitar.wav`
next to `NamToClo.exe`. The source WAV is loaded into RAM and resampled once to
the NAM model's expected sample rate. Playback is then streamed in small blocks
through the already loaded NeuralAmpModelerCore DSP; no processed preview WAV is
generated before playback.

`Replay preview` resets the NAM and starts the source from the beginning. `Stop`
stops the streaming playback.

The v2.9.1 build uses the standard Windows multimedia output selected as the
